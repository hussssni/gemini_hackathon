...
const optionRecord = z.object({
  ...
  state: z.enum(["untried", "way-back", "dead-end", "loops-back", "exhausted", "open"]),
...
/**
 * The HTTP app. `generate` is the Gemini call, injected so the route can be
 * exercised end to end without spending quota.
 */
export function createApp({ generate = generateJson } = {}) {
  const app = express();
  app.use(express.json({ limit: "12mb" }));
  app.use(express.static("public"));
  app.post("/api/survey", (req, res) => handleSurvey(req, res, generate));
  return app;
}
